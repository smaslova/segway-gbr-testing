...
for run_num in range(1, nruns+1):
    ...
    rev_seg = pd.read_csv("../segway/out%s/rev_posteriordir%s/segway.bed"%(stdev,run_num), sep="\t",skiprows=1, header=None)
    ...
    rev_gbr = pd.read_csv("../segway-gbr/segway_gbr/out%s/REV_POSTDIR_W%s_B%s_%s/segway.bed"%(stdev,weight, beta, run_num), sep="\t",skiprows=1, header=None)
    ...
    rev_seg_pred = get_label_numbers(np.flip(transform_pred(rev_seg)), data['track1'].values)

    gbr_pred = get_label_numbers(transform_pred(gbr), data['track1'].values)
    rev_gbr_pred = get_label_numbers(np.flip(transform_pred(rev_gbr)), data['track1'].values)

    #calculate difference
    seg_diff_all.append(get_squared_error(seg_pred, rev_seg_pred))
    sgbr_diff_all.append(get_squared_error(gbr_pred, rev_gbr_pred))

diff = pd.DataFrame({'segway':seg_diff_all, 'segway-gbr':sgbr_diff_all})
diff.to_csv("./outfiles/reverse_diff_stdev%s_b%s_w%s.csv" % (stdev, beta, weight))

sns.boxplot(data=diff)
plt.savefig("./figures/rev_diff_boxplot%s_b%s_w%s.png"%(stdev, beta, weight)) 
plt.close()

sns.scatterplot(x='segway', y='segway-gbr', data=diff)
plt.savefig("./figures/rev_diff_scatterplot%s_b%s_w%s.png"%(stdev, beta, weight))
plt.close()
