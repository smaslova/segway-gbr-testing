...
for i, stdev in enumerate(stdevs_tried):
    file = "./outfiles/reverse_diff_stdev%s_b%s_w%s.csv" % (stdev, beta, weight)
...
plt.tight_layout()
plt.savefig("./figures/combined/MR2_barplot_b%s_w%s.pdf"%(beta, weight))
plt.close()
...
plt.ylabel("Mean Error")
plt.tight_layout()
plt.savefig("./figures/combined/MR2_boxplot_b%s_w%s.pdf"%(beta, weight))
plt.close()